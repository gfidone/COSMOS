...
class User():
    ...
    def generate(self, topic=None, parent=None, root=None, active_stream=True, generation_config=None, seed=None):
        if topic:
            result = {'l_prompt': self.unbiased_post_prompt(**self.profile, topic=topic)}
            result['a_prompt'] = self.biased_post_prompt(**self.profile, topic=topic, intervention=self._format_interventions()) if self.interventions else result['l_prompt']
        else:
            a_root_content = root['a_content'] if root['a_content'] is not None else root['l_content']
            a_parent_content = parent['a_content'] if parent['a_content'] is not None else parent['l_content']
            l_thread = f'Post by {root["username"]}: "{root["l_content"]}"\n' if parent == root else f'Post by {root["username"]}: "{root["l_content"]}"\nComment by {parent["username"]}: "{parent["l_content"]}"'
            ...
            result = {'l_prompt': self.unbiased_comment_prompt(**self.profile, thread=l_thread)}
            result['a_prompt'] = self.biased_comment_prompt(**self.profile, thread=a_thread, intervention=self._format_interventions()) if self.interventions else result['l_prompt']
        result['l_output'], result['l_tags'], result['l_content'] = self.pipeline.inference(result['l_prompt'], generation_config, seed=seed)
        if active_stream and result['l_prompt'] == result['a_prompt']: 
            result['a_output'], result['a_tags'], result['a_content'] = result['l_output'], result['l_tags'], result['l_content']
        else: 
